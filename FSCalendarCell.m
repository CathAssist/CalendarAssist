...
//======modified by Pat
    NSArray *ymd = [Constants getYMDFromDate:self.date];
    NSArray* events = [Liturgic getLitugicDay:[ymd[0] intValue]:[ymd[1] intValue]:[ymd[2] intValue]];
    if ([events count] > 0)
    {
        NSString *colorStr = [events objectAtIndex:0];
        UIColor *c;
        if ([colorStr isEqualToString:@"红"])
            c = [UIColor redColor];
        ...
        _eventLayer.fillColor = c.CGColor;
        _eventLayer.strokeColor = [UIColor  lightGrayColor].CGColor;
    }
//======modified by Pat
